...
if (isset($_GET['editt'])){
    ...
    INNER JOIN emp_credit AS cre ON cre.account_id = emp.account_id
    INNER JOIN (SELECT account_id,leave_id,cre_vbalance,cre_sbalance,MAX(cre_id) AS cre_id FROM emp_credit GROUP BY cre_id) AS dt 
    ON dt.account_id = lea.account_id 
    INNER JOIN account ON account.Id = emp.account_id
    WHERE lea.leave_id = '$leave_id'");
    $account->execute();
    $cre_id = $conn->lastInsertId();
...
if (isset($_POST['Comfirmleave'])){
    include_once "assets/connection.php";
    $conn = dbConnect();
  $account_id =$_POST['Id'];
    $leave_id=$_POST['leave_id'];
    $leave_status =implode($_POST['leave_status']);
    $leave_status_due =implode($_POST['leave_status_due']);
    $cre_vbalance1 =$_POST['cre_vbalance'];
    $cre_sbalance1 =$_POST['cre_sbalance'];
    $leave_days =$_POST['leave_days'];
    $leave_days1 =$_POST['leave_days'];
    $leave_type=implode($_POST['leave_type']);
    $leave_from =$_POST['leave_from'];
    $leave_to =$_POST['leave_to'];
    ...
    if($leave_status_due =='Days with pay' && $leave_type =='Vacation'){
        $cre_vwpay =$leave_days;
    }else if($leave_status_due =='Days without pay' && $leave_type =='Vacation'){
        $cre_vwopay =$leave_days;
    }else if($leave_status_due =='Days with pay' && $leave_type =='Sick'){
        $cre_swpay =$leave_days;
    }else if($leave_status_due =='Days without pay' && $leave_type =='Sick'){
        $cre_swopay =$leave_days;
    }else{

    }

    if($leave_type =='Vacation'){
        $cre_vbalance =$cre_vbalance1 -$leave_days;
    }else if($leave_type =='Sick'){
        $cre_sbalance =$cre_sbalance1 -$leave_days;
    }else{
    ...
    if($leave_status =='Approved'){
        ...
        leave_status='$leave_status',leave_status_due='$leave_status_due' WHERE leave_id ='$leave_id'");
        $account->execute(); 

        $credit =$conn->prepare("INSERT INTO emp_credit (cre_days,cre_vwpay, cre_vwopay,cre_vbalance,cre_swpay, cre_swopay,cre_sbalance,cre_type,cre_datefrom,cre_dateto,cre_status,leave_id,account_id)
        VALUES('$leave_days1','$cre_vwpay','$cre_vwopay','$cre_vbalance','$cre_swpay','$cre_swopay','$cre_sbalance','$leave_type','$leave_from ','$leave_to','$leave_status','$leave_id','$account_id')");
        $credit->execute(); 

    }else if($leave_status =='Not Approved'){
        $account = $conn->prepare("UPDATE  emp_leave SET 
        leave_status='$leave_status',leave_status_due='$leave_status_due' WHERE leave_id ='$leave_id'");
        $account->execute(); 
    }else{
    ...
    {  
        ...
        Swal.fire({
            ...
            timer: 1500
                ...
                window.location = "leave_profile.php?editt=<?php echo$leave_id?>";
        }); 
</script>
                  <?php
    }
