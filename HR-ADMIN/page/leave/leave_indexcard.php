            ...
            <table class="table table-bordered  leaveSta " id="dataTable" width="100%" cellspacing="0">
                ...
                <tbody>
                    <?php
                        ...
                        $conn = dbConnect();
                        $account = $conn->prepare("SELECT * FROM emp_credit  WHERE account_id = $_SESSION[Id] AND cre_status='Approved'");
                        $account->execute();
  ...
  <div class="card-body">
                ...
                <thead>
                            ...
                            <?php
                                ...
                                include_once "assets/connection.php";
                                $conn = dbConnect();
                                $account = $conn->prepare("SELECT * FROM emp_credit WHERE account_id = $_SESSION[Id] AND month_date !=''
                                ");
                                $account->execute();
                                while($result = $account->fetch(PDO::FETCH_ASSOC)){

                                    $month_name = $result["month_date"];
                                    $month_name1 =date("F", strtotime($month_name));
